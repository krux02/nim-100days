...
proc valprint(s: string, value: float) =
   ...
   echo(s.align(30), ": ", value.formatFloat(ffScientific, 10))
...
proc main() =
   ...
   let
      ...
      sigma_iso = 1.0/3.0*tr(sigma)*eye(3)
      sigma_dev = sigma - sigma_iso

   # compute principal stresses
   var eigvals = eigenvalues(sigma).real
   ...
   let
      I1 = tr(sigma)
      J2 = 1.0/2.0*tr(sigma_dev*sigma_dev)
      J3 = 1.0/3.0*tr(sigma_dev*sigma_dev*sigma_dev)
