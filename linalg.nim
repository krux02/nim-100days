...
type
  OrderType* = enum
    rowMajor, colMajor
  Vector*[N: static[int]] = ref array[N, float64]
  Matrix*[M, N: static[int]] = object
    order: OrderType
    data: ref array[M * N, float64]
  Array[N: static[int]] = array[N, float64]
  DoubleArray[M, N: static[int]] = array[M, array[N, float64]]
...
proc at*[M, N](m: Matrix[M, N], i, j: int): float64 {.inline.} =
  let data = cast[ref DoubleArray[N, M]](m.data)
  data[j][i]

template `[]`*(m: Matrix, i, j: int): float64 = m.at(i, j)
...
proc trace*[N](m: Matrix[N, N]): float64 =
  for i in 0 .. < N:
    result += m.data[i * (1 + N)]
...
proc det*(m: Matrix[3, 3]): float64 =
  + m[0,0] * (m[1,1] * m[2,2] - m[2,1] * m[1,2]) -
    m[1,0] * (m[0,1] * m[2,2] - m[2,1] * m[0,2]) +
    m[2,0] * (m[0,1] * m[1,2] - m[1,1] * m[0,2])

proc eigvals*(m: Matrix[3, 3]): tuple[eig1, eig2, eig3: float64] =
